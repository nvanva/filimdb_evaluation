...
def predict_probs(module, model, id_to_word, data, unigram_probs, name, top_k=3, bs=100):
    data = np.array(data)
    tail = len(data)//bs*bs
    ...
    for cur_id, next_id, (softmax, _) in zip(tqdm(X, desc=desc), Y, module.next_proba_gen(X_protected_it, model)):
    ...
    for cur_id, next_id, (softmax, _) in zip(tqdm(X_tail, desc=desc), Y_tail, module.next_proba_gen(X_protected_it, model)):
        X_protected_it.allow_next = True
        append_prediction(preds, cur_id, next_id, softmax, unigram_probs, id_to_word, top_k, bs)
...
class Evaluator:
    ...
    def evaluate(self, ptb_path=PTB_PATH):
        ...
        model = train(lm, ds, None)
        scores, allpreds = test(lm, ds, model)
...
def test(module, ds, model):
    ...
    allpreds.extend(predict_probs(module, model, id_to_word, train_data, unigram_probs, 'train'))
    allpreds.extend(predict_probs(module, model, id_to_word, dev_data, unigram_probs, 'valid'))
    allpreds.extend(predict_probs(module, model, id_to_word, test_data, unigram_probs, 'test'))
    # TODO: refactor score_preds to get rid of saveing preds to files in order to score predicions if possible!
    with NamedTemporaryFile(delete=True) as fpreds_tmp:
        save_preds(allpreds, preds_fname=fpreds_tmp.name)
        scores = score_preds(fpreds_tmp.name, ptb_path)
    print(datetime_str(), 'Testing model finished.')


    return scores, allpreds
